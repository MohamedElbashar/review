...
export class Producer {
    ...
    private __attachCloseAndErrorListeners(
        entity: Connection | Channel,
    ...
    async closeChannels() {
        ...
        await Promise.all(
            channelClosePromiseArr.map((p, index) => ({
                index,
                error: p.catch(err => err),
            }))
        ).then(res => {
            res.forEach(o => {
                if (!o.error) {
    ...
    private log(level: LogLevel, msg: string, ...args) {
        log[level](
            `[AMQP] ${msg}${
                args.length ? args.map(e => safeStringify(e)).join('\n') : args
            }`
        );
    }
}
