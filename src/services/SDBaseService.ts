...
export class SDBaseService {
    ...
    httpRequest(
        ...
        opts = this.setRedirects(opts, followRedirects);
        ...
        return got(opts).then(res => {
            ...
            if (msg['headers'].hasOwnProperty('set-cookie')) {
                msg['responseCookies'] = this.extractCookies(
    ...
    setRedirects(opts, followRedirects) {
    ...
    setCookies(opts, cookies, url) {
        if (opts.headers.hasOwnProperty('cookie')) {
            cookies = cookie.parse(opts.headers.cookie, { decode: String });
    ...
    getMiddlesWaresBySequenceId(
        ...
        if (Array.isArray(middleWares)) {
            middleWares.forEach(middleware => {
                ...
                const mwName = middleware[serviceName];
                if (
                ...
                if (generatedMiddleWares?.[serviceName][mwName]) {
                    returnedMws.push(
                        generatedMiddleWares[serviceName][mwName].functionDef
                    );
                }
            });
        }
    ...
    buildDigestHeader(user, pass, method, path, authHeader) {
        ...
        const qop = /(^|,)\s*auth\s*($|,)/.test(challenge['realm']) && 'auth';
        const nc = qop && '00000001';
        const cnonce = qop && uuid().replace(/-/g, '');
        const ha1 = this.ha1Compute(
            challenge['algorithm'],
            user,
            challenge['realm'],
            pass,
