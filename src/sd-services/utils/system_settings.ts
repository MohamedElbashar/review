...
export class system_settings {
  ...
  async updateAllSystemSettings(
    ...
    try {
      bh = this.sdService.__constructDefault(bh);
      this.tracerService.sendData(spanInst, bh);
      bh = await this.sd_FdvCxs3LxtChIUdW(bh, parentSpanInst);
  ...
  async getCache(bh, parentSpanInst) {
    ...
    try {
      ...
      this.tracerService.sendData(spanInst, bh);
      bh = await this.sd_BHvf7aSaMsjCJMem(bh, parentSpanInst);
  ...
  async sd_BHvf7aSaMsjCJMem(bh, parentSpanInst) {
    const spanInst = this.tracerService.createSpan(
      'sd_BHvf7aSaMsjCJMem',
      parentSpanInst
    );
  ...
  async httpSystemSettingList(bh, parentSpanInst) {
    try {
      ...
      return await this.errorHandler(bh, e, 'sd_t52zZppvTdKfBbyA');
    }
  }
  ...
  async setUrl1(bh, parentSpanInst) {
    ...
    try {
      ...
      this.tracerService.sendData(spanInst, bh);
      bh = await this.sd_gd8y7c9xJv6IDAUC(bh, parentSpanInst);
  ...
  async sd_gd8y7c9xJv6IDAUC(bh, parentSpanInst) {
    try {
      let requestOptions: any = {
        url: bh.url,
        timeout: 30000,
      ...
      //appendnew_next_sd_gd8y7c9xJv6IDAUC
      return bh;
    } catch (e) {
      return await this.errorHandler(bh, e, 'sd_gd8y7c9xJv6IDAUC');
    }
  }
  ...
  async sd_FdvCxs3LxtChIUdW(bh, parentSpanInst) {
    const spanInst = this.tracerService.createSpan(
      'sd_FdvCxs3LxtChIUdW',
      parentSpanInst
    );
    try {
      ...
      bh.local.status =
        result_arr.filter(
          ...
          : false;
      this.tracerService.sendData(spanInst, bh);
      //appendnew_next_sd_FdvCxs3LxtChIUdW
      return bh;
    } catch (e) {
      return await this.errorHandler(
        bh,
